...
class LockParserBase {
    ...
    async getDependencyTree(manifestFile, lockfile, includeDev = false, strict = true, workspace) {
        ...
        const topLevelDepsResult = await (0, _1.getTopLevelDeps)(manifestFile, includeDev, lockfile, workspace);
        const topLevelDeps = topLevelDepsResult.dependenciesArray;
        ...
        let depMap;
        if (this.type === 'pnpm') {
            depMap = this.getDepMap(yarnLock, topLevelDepsResult.pnpmDependencies, topLevelDepsResult.pnpmDevDeps);
        }
        else {
