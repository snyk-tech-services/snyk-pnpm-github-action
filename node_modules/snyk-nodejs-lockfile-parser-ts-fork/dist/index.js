...
exports.OutOfSyncError = exports.InvalidUserInputError = exports.UnsupportedRuntimeError = exports.LockfileType = exports.Scope = exports.getPnpmWorkspaces = exports.getPnpmWorkspacesFromFiles = exports.getYarnWorkspaces = exports.getYarnWorkspacesFromFiles = exports.buildDepTreeFromFiles = exports.buildDepTree = void 0;
const fs = require("fs");
...
const pnpm_lock_parser_1 = require("./parsers/pnpm-lock-parser");
...
async function buildDepTree(manifestFileContents, lockFileContents, includeDev = false, lockfileType, strict = true, defaultManifestFileName = 'package.json', workspace) {
    if (!lockfileType) {
    ...
    else if (lockfileType === parsers_1.LockfileType.pnpm) {
        lockfileType = parsers_1.LockfileType.pnpm;
    }
    let lockfileParser;
    switch (lockfileType) {
        ...
        case parsers_1.LockfileType.yarn2:
            ...
            break;
        case parsers_1.LockfileType.pnpm:
            lockfileParser = new pnpm_lock_parser_1.PnpmPackageLockParser();
            break;
        default:
    ...
    const lockFile = await lockfileParser.parseLockFile(lockFileContents);
    return lockfileParser.getDependencyTree(manifestFile, lockFile, includeDev, strict, workspace);
}
exports.buildDepTree = buildDepTree;
async function buildDepTreeFromFiles(root, manifestFilePath, lockFilePath, includeDev = false, strict = true, workspace) {
    ...
    return await buildDepTree(manifestFileContents, lockFileContents, includeDev, lockFileType, strict, manifestFilePath, workspace);
}
...
function getPnpmWorkspacesFromFiles(root, pnpmWorkspaceFilePath) {
    if (!root || !pnpmWorkspaceFilePath) {
        throw new Error('Missing required parameters for getPnpmWorkspacesFromFiles()');
    }
    const pnpmWorkspaceFileFullPath = path.resolve(root, pnpmWorkspaceFilePath);
    if (!fs.existsSync(pnpmWorkspaceFileFullPath)) {
        throw new errors_1.InvalidUserInputError(`Target file not found at : ${pnpmWorkspaceFileFullPath}`);
    }
    const pnpmWorkspaceFileContents = fs.readFileSync(pnpmWorkspaceFileFullPath, 'utf-8');
    return (0, parsers_1.getPnpmWorkspaces)(pnpmWorkspaceFileContents);
}
exports.getPnpmWorkspacesFromFiles = getPnpmWorkspacesFromFiles;
